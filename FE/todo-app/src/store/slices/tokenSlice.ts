...
export const tokenSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase<string, HydrateCookieAction>(
      HYDRATE,
      (state, { payload }) =>
    ...
    builder.addMatcher(
      ...
      (_state, { payload }) => payload.data.token
    );
    builder.addMatcher(
    ...
    builder.addMatcher(
      authenticationApi.endpoints.completeRegistration.matchFulfilled,
      (_state, { payload }) => payload.data.token
    );
    builder.addMatcher(
      authenticationApi.endpoints.completeReset.matchFulfilled,
      (_state, { payload }) => payload.data.token
    );
    builder.addMatcher(
    ...
    builder.addMatcher(
      ...
      (state, { meta }) => (meta.condition ? state : null)
    );
